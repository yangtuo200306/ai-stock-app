...
### 职业目标
- **目标岗位：AI 应用开发工程师**
- 核心能力：使用 AI 编程工具构建产品，理解 AI 应用架构（Prompt 工程、RAG、Agent、流式响应）
- 前端/后端知识：辅助理解，不是主攻方向
- 工程化实践：理解概念和为什么重要，具体实现由 AI 协助完成

### 已掌握的基础能力
已完成 v0.1 ~ v0.9 的基础学习阶段，已掌握：
...
### 当前阶段
进入 **v1.8 股票数据体系优化阶段**，按总体方案（`docs/overall-roadmap.md`）推进：

1. **统一数据结构与类型定义** — 对齐前后端数据字段，标准化类型系统
2. **优化数据展示** — 自选列表/问股结果/记录详情展示增强
3. **完善指标与数据源评估** — 增删调整指标，评估是否需要加新数据源

学习方式：每做一步，同步讲解背后的工程原理和生产级实践，让你边做边学。
用户仍重视"理解为什么"，可以接受更深入的工程化讨论，不再需要从零开始的解释。
...
按总体方案（`docs/overall-roadmap.md`）分阶段推进：

**阶段一（全局架构升级）：**
- 前端状态管理升级：Zustand 设计思想、Store 拆分、组件按需订阅
- API 层增强：统一错误处理、401 自动处理
- 后端配置管理：集中式 Settings 类、环境变量管理
- 后端日志系统：分级日志、文件轮转
- 错误码体系：结构化 error_code、错误中间件
- 数据库迁移：版本化迁移机制
...
**v1.8（股票数据体系优化）：**
- 统一前后端数据结构与类型定义
- 优化自选列表/问股结果/记录详情的数据展示
- 完善技术指标，评估数据源扩展
- 参考项目数据展示设计借鉴

**贯穿全程：**
- AI 应用开发核心能力：Prompt 工程、RAG 模式、Agent 架构、流式响应
- AI 协作开发能力升级：上下文工程、MCP、Skill、Agent 工作流

## 11. 计划模式质量门禁

涉及删除/替换公共模块（Context、Provider、Hook、工具函数等）时，输出计划前必须执行以下流程，并在计划中附上扫描结果供用户验证：
...
- 新增文件 → 检查是否需要更新 index.ts / __init__.py 等入口导出
- 新增 API 路由 → 检查 main.py 是否注册 router
- 新增导航页面 → 检查导航类型定义和 Stack/Tab 注册
...
- 是否对使用方数量做了假设？（必须验证）
- 是否遗漏了入口文件的更新？（必须检查）
- 重构时的 import 路径是否都正确？（必须验证）
- 计划中的复杂步骤是否粒度足够细？（拆到原子操作）
...
- 对调研记忆中的关键细节（配置项、import、函数签名、文件路径等），必须重新读取文件验证，不能仅凭记忆下结论
- 涉及修改的文件，出计划前至少确认一次当前实际内容

### 13.2 执行阶段

- 执行完成后，启动服务并调用至少一个涉及修改的接口，确认运行时无报错
