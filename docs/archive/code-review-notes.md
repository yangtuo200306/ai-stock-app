...
- 当前是否需要立即修：已修复（v1.4 改为从 `src/constants/app.ts` 读取 `APP_VERSION`）。
- 后续建议：后续升级版本时只需修改 `src/constants/app.ts` 一处。
...
- 影响：用户可能误触删除。
- 当前是否需要立即修：已修复（v1.2 新增 `ConfirmDialog` 组件，删除前弹出确认弹窗）。
- 后续建议：后续可复用 `ConfirmDialog` 到其他删除场景。
...
- 当前是否需要立即修：已修复（v1.0 引入 Zustand `watchlistStore`，两页共享同一 store，数据自动同步）。
- 后续建议：无。
...
> **v1.1 更新：** 问题 124（自动滚动）✅ 已修复，问题 129（复制+Markdown）✅ 已修复。问题 125（重试入口）部分实现，待后续完善。
